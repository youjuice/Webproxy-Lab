...
int open_listenfd(char *port)               
{
    ...
    hints.ai_flags = AI_PASSIVE | AI_ADDRCONFIG; /* ... on any IP address - AI_PASSIVE를 설정하면 듣기 소켓으로 이용 가능 / AI_ADDRCONFIG를 설정하면 사용 가능한 ip 주소만 가져옴 */
    ...
    for (p = listp; p; p = p->ai_next) {
        ...
        setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, (const void *)&optval , sizeof(int));    // optval -> SO_REUSEADDR 옵션 활성화를 위한 변수 (1로 설정)
    ...
    /* Clean up */
    freeaddrinfo(listp);                // addrinfo 구조체 반환
